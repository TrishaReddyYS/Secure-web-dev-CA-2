ScholarHub
A secure web application for managing student courses, assignments, and submissions with enterprise-level security features. git
...
Threat Model
SQL Injection: Mitigated through Prisma ORM parameterized queries
XSS Attacks: Prevented via input validation and React's built-in escaping
CSRF: Mitigated with SameSite cookies and secure session management
Brute Force: Rate limiting should be implemented (future enhancement)
Session Hijacking: Prevented with HTTP-only, secure cookies
Privilege Escalation: Role-based checks on every protected operation
Data Leakage: Generic error messages, no sensitive data in responses
Known Limitations
...
Account Lockout: Not implemented after failed login attempts
File Upload: Not implemented - would need additional security measures
...
Proper error handling with generic client messages
...
👥 Author
Trisha Yennam Showri Student ID: 24243426 MSc in Cybersecurity - National College of Ireland
